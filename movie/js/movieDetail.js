;(function($){
	...
	$.fn.movieScroller = function(options){
		...
		if( $("#data_table_body tr").length>2 ){
...
String.prototype.fmtWithZero=function(isFmtWithZero){　　
		return(isFmtWithZero&&this.length==1)?"0"+this:this;
}
...
function initPlayLisTable(dataArray){
	if( dataArray=='undefined'||dataArray==null||dataArray.length==0 ){
		var html ='<tr><td colspan=\"10\">影院当前还没有更新排片！..你可以: <a class=\"c_red pl4\" href=\"movie.html\">选择其他影院或其他日期</td></tr>';
		...
		for( var i = 0 ; i < dataArray.length ; i++ ){
			var info = dataArray[i];
 			html = html+'<tr name=\"planInfo\" date=\"'+info.featureDate+'\" place=\"'+info.placeNo+'\">';
			html = html+'<td class=\"w86\" >'+info.featureTime+'</td>';
			html = html+'<td class=\"w110\">'+info.copyLanguage+'&nbsp;'+info.copyType+'</td>';
			html = html+'<td class="w110">'+info.hallName+'</td>';
			html = html+'<td class=\"w110\">(<s>'+info.standPric+'</s>)'+'<span class=\"c_red pl4\">'+info.aPrice+'</span>'+'<td>';
			html = html+'<td class=\"w130\">'+info.explain+'<td>'
			html = html+'<td><a  style=\" margin-left: 55px;\"  class=\"select_btn\" href=\"cinema\/'+info.featureAppNo+'\/'+info.aPrice+'\/planSite.html\">选座购票</a><td>';
			html = html+'</tr>'
		}
		$("#data_table_body").empty().append(html);
